...
const AnswersAndResponeses = () => {
  ...
  const [local,setLocal]=useState(JSON.parse(localStorage.getItem("optionstore")))
  ...
  return (
        <div className='main' id="main">
          ...
          <p className='goto-top' style={style}>  <a href="#main">Go to top</a></p>
            {
                 ...
                 { Data[item1].map((item2,index2)=>{
                     return ( 
                      <div className="question-card" key={index2}> 
                         ...
                         {("a"==local[`${item1}`.split("").at(-1)][index2])?<p className='answered-option'>{`a.) ${item2["a"]}`}</p>:<p>{`a.) ${item2["a"]}`}</p>}
                         {("b"==local[`${item1}`.split("").at(-1)][index2])?<p className='answered-option'>{`b.) ${item2["b"]}`}</p>:<p>{`b.) ${item2["b"]}`}</p>}
                         {("c"==local[`${item1}`.split("").at(-1)][index2])?<p className='answered-option'>{`c.) ${item2["c"]}`}</p>:<p>{`c.) ${item2["c"]}`}</p>}
                         {/* <p>{`c.) ${item2["c"]}`}</p> */}
                         <Divider/>
